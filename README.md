...
- Backend (FastAPI):
  ...
  ```bash
  nix develop .#backend --command bash -lc "cd api && source .venv/bin/activate && fastapi dev app/main.py --port 8000"
  ```
...
- ngrok (expose API on 8000):

  ```bash
  # optional: close existing ngrok processes first
  pkill -x ngrok || true
  ngrok http 8000
  ```
